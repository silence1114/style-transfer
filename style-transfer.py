...
def face_classify(input_im):
    ...
    if len(faces)>0:#如果人脸数组长度大于0
        for face in faces: #对每一个人脸画矩形框
                x, y, w, h = face
                cv2.rectangle(input_im_copy, (x, y), (x+w, y+h), (0,0,0))
...
def face_correct(input_im,faces):
    ...
    for face in faces:
        ...
        print(luminance_med,luminance_th)
        # 亮度补偿
        if luminance_med > luminance_th:
           ...
           luminance_corr = ((1-w)*tmp_box_luminance+w*(np.power(tmp_box_luminance, gamma_corr)))*mask+(1-mask)*im_box_luminance
